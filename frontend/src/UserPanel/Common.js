...
export function AddApply(dict, type) {
  var uid = window.sessionStorage.getItem('id');

  return fetch(API_URL + '/addapply/', {
    method: 'POST',
    body: JSON.stringify({ ...dict, curusr: uid, type: type }),
    headers: {
      'Content-type': 'application/json; charset=UTF-8'
    }
  })
...
export function ApproveApply(id, type) {
  var uid = window.sessionStorage.getItem('id');

  return fetch(API_URL + '/approveapply/', {
    method: 'POST',
    body: JSON.stringify({ id: id, type: type, curusr: uid }),
    headers: {
      'Content-type': 'application/json; charset=UTF-8'
    }
  })
...
export function RejectApply(id, type) {
  ...
  return fetch(API_URL + '/rejectapply/', {
    method: 'POST',
    body: JSON.stringify({ id: id, type: type, curusr: uid }),
    headers: {
      'Content-type': 'application/json; charset=UTF-8'
